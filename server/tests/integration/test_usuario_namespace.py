...
class TestUsuario(BaseTest):
    ...
    def test_get_all_usuario_200(self, *criar_dominio):
        with self.app() as client:
            with self.app_context():
                client.post(f'{BASE_URL}/usuario', json=PAYLOAD)
                request = client.get(f'{BASE_URL}/listar_usuarios')
    ...
    def test_delete_usuario_200(self, *criar_dominio):
        with self.app() as client:
            with self.app_context():
                client.post(f'{BASE_URL}/usuario', json=PAYLOAD)
                request = client.delete(f'{BASE_URL}/usuario', json=PAYLOAD)

                request_get = client.get(f'{BASE_URL}/usuario')

                self.assertEqual(request.status_code, 200)
                self.assertEqual(json.loads(request_get.text)['ativo'], False)
